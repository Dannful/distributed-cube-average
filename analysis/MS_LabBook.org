...
tibble(FILE = dir_ls(THE.DIR, pattern="*.csv")) |>
  ...
  mutate(SUMMARY.TIME.PLOT = map(DATA, plot_total_time_per_operation)) |>
  mutate(SUMMARY.COMBINED = map2(SUMMARY.PLOT, SUMMARY.TIME.PLOT, `|`)) |>
  mutate(DATA.AGG = map(DATA, do_the_reference_aggregation, 50)) |>
  # tradicional ST view
  mutate(ST = map(DATA, our_st_plot)) |>  
  mutate(ST.AGG = map(DATA.AGG, \(x) do_the_viz(x, has.summary = ""))) -> df
#+end_src
...
df |>
  select(FILE, ST.AGG) |>
  pwalk(function(FILE, ST.AGG) {

    page <- (ST.AGG)
    print(page + ggtitle(FILE))# + coord_cartesian(xlim=c(0,1)))
