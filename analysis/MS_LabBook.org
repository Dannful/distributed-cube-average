...
** 2026-04-03
:PROPERTIES:
:header-args: :tangle "do.R" :tangle-mode (identity #o755) :shebang "#!/usr/bin/Rscript"
:END:
...
#+RESULTS:
**** StarVZ adapted functions so they can be faster

#+begin_src R :results output :session *R* :exports both :noweb yes :colnames yes
do_the_aggregation <- function(df, step_width) {
  df |>
    rename(Task = Value) |> # Task is the Operation (MPI_Send)
    rename(ResourceId = Container) |> # ResourceId is the Process (Container, Rank, Thread)
    group_by(ResourceId, Task) |>
    mutate(Value = 1) -> dfw_initial
...
# Define the first zero
  dfw_initial |>
    ...
    mutate(StartN = 0) |>
    mutate(EndN = Start) |>
    mutate(Value = 0) -> dfw_zero_1

# Define other zeroes
  dfw_initial |>
    mutate(StartN = End) |>
    mutate(EndN = lead(Start)) |>
    mutate(Value = 0) -> dfw_zero_N

# Row bind them
  bind_rows(dfw_zero_1, dfw_zero_N) |>
    mutate(Start = StartN) |>
    mutate(End = EndN) |>
    select(-StartN, -EndN) |>
    bind_rows(dfw_initial) |>
    ungroup() -> dfw_agg_prep

# Set max end time for NA cases
  dfw_agg_prep %>%
    filter(!complete.cases(.)) |>
    mutate(End = max(dfw_initial$End)) %>%
    bind_rows(dfw_agg_prep %>% filter(complete.cases(.))) %>%
    mutate(Duration = End - Start) |>
    arrange(ResourceId, Task, Start) -> dfw_agg_prep_ready
...
do_the_reference_aggregation <- function(df, nb_intervals) {
  ...
  df |>
    group_by(Container) |> # aggregate per process (Container, Rank, Thread)
...
our_read_function <- function(f) {
  read_csv(f,
    ...
    filter(Value %in% c("MPI_Irecv", "MPI_Isend", "MPI_Waitall")) ->df
...
tibble(FILE = dir_ls("2026-04-03", pattern="*.csv")) |>
  ...
  mutate(DATA.AGG = map(DATA, do_the_reference_aggregation, 5)) |>
  # tradicional ST view
  # mutate(ST = map(DATA, our_st_plot)) |>  
  # mutate(ST = map(ST, ~ .x + coord_cartesian(
  #   xlim = range(unlist(map(DATA, ~ c(.x$Start, .x$End))), na.rm = TRUE)
  # ))) |>
  slice(1) |>
  unnest(DATA.AGG) |>
  select(-FILE, -KEY, -DATA) |>
  filter(Slice == 0) |>
  filter(Container == 1) |>
  print()
#  mutate(ST.AGG = map(DATA.AGG, do_the_viz, has.summary=TRUE)) |>
#  slice(1) |>
#  pull(ST.AGG)
#+end_src
...
#+RESULTS:
: # A tibble: 3 × 8
:   Container Value       Slice   Percent StartTS EndTS DurationTS Time.Sum
:       <int> <fct>       <dbl>     <dbl>   <dbl> <dbl>      <dbl>    <dbl>
: 1         1 MPI_Irecv       0 0.585           0  49.9       49.9 29.2    
: 2         1 MPI_Isend       0 0.0000752       0  49.9       49.9  0.00375
: 3         1 MPI_Waitall     0 0.777           0  49.9       49.9 38.7
