...
if (nrow(df) > 0) {
    df <- df |>
        ...
        summarise(total_time = mean(total_time), msamples_per_s = mean(msamples_per_s))

    plot <- ggplot(data = df, mapping = aes(x = problem_size, y = msamples_per_s)) + geom_line() +
        geom_point() + theme_minimal() + ylab("MSamples/s") + xlab("Problem Size")
    print(plot)
    print(df)
} else {
