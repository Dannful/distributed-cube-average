      ...
      in {
        ...
        packages.script =
          pkgs.writeShellScriptBin "run-distributed-cube-average" ''
            ${pkgs.openmpi}/bin/mpirun --map-by :OVERSUBSCRIBE -np 8 ${distributed-cube-average}/bin/distributed-cube-average --size-x=10 --size-y=10 --size-z=10 --absorption=6 --dx=2 --dy=3 --dz=4 --dt=1 --time-max=1 --output-file=./validation/predicted.dc
          '';
