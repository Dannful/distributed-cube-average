...
print(paste("Detected Paje output in:", input_file))
...
df.states |>
    ...
    scale_fill_manual(values = c(MPI_Irecv = "#FCA5A7", MPI_Isend = "#A6C8E5", MPI_Recv = "#E41A1C",
        MPI_Send = "#377EB8", MPI_Waitall = "#FF7F00"), breaks = c("MPI_Recv", "MPI_Irecv",
        "MPI_Send", "MPI_Isend", "MPI_Waitall")) + geom_text(data = data.frame(x = makespan,
    y = (max_rank - min_rank)/2), aes(x = x, y = y, label = paste0(round(makespan,
    0))), inherit.aes = FALSE, angle = 90, size = 2.5) + theme_bw(base_size = 10) +
    guides(fill = guide_legend(nrow = 1)) + theme(plot.margin = unit(c(0, 0, 0, 0),
    "cm"), legend.margin = margin(t = 0, unit = "cm"), panel.grid = element_blank(),
