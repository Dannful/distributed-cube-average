...
# Determine file type Read file to detect format We read the whole file (or
# enough of it) because mpiP output might be at the end of a log
...
if (is_mpip) {
    ...
    read_csv(input_file, show_col_types = FALSE, col_names = c("Nature", "Container",
    ...
    if (nrow(df.overall) > 0) {
        # Check for MPI_Send operations first to avoid max() warning on empty
        # set
        df.sends <- df.overall |>
    ...
    df.states |>
        ...
        ggplot() + geom_hline(yintercept = seq(min(df.states$Rank) + 0.5, max(df.states$Rank) +
        1 - 0.5, by = 1), color = "grey85", linewidth = 0.1) + geom_rect(aes(xmin = Start,
        xmax = End, ymin = Rank, ymax = Rank + 0.9, fill = Operation)) + scale_y_continuous(breaks = mid_breaks,
        labels = seq(min_rank, max_rank, by = 1)) + xlab("Time [seconds]") + ylab("Rank [count]") +
        scale_fill_manual(values = c(MPI_Irecv = "#FCA5A7", MPI_Isend = "#A6C8E5",
            MPI_Recv = "#E41A1C", MPI_Send = "#377EB8", MPI_Waitall = "#FF7F00"),
            breaks = c("MPI_Recv", "MPI_Irecv", "MPI_Send", "MPI_Isend", "MPI_Waitall")) +
        geom_text(data = data.frame(x = makespan, y = (max_rank - min_rank)/2), aes(x = x,
            y = y, label = paste0(round(makespan, 0))), inherit.aes = FALSE, angle = 90,
            size = 2.5) + theme_bw(base_size = 10) + guides(fill = guide_legend(nrow = 1)) +
        theme(plot.margin = unit(c(0, 0, 0, 0), "cm"), legend.margin = margin(t = 0,
            unit = "cm"), panel.grid = element_blank(), legend.position = "top",
            legend.justification = "left", legend.box.spacing = unit(1, "pt"), legend.box.margin = margin(0,
                0, 0, 0), legend.title = element_blank()) -> plot
    ...
    saveWidget(widget = interactive_plot, file = "smpi.html", selfcontained = TRUE)
}
