...
void randomVelocityBoundary(int sx, int sy, int sz, int nx, int ny, int nz,
  ...
  for (iz = bord + absorb; iz < nz + bord + absorb; iz++) {
    for (iy = bord + absorb; iy < ny + bord + absorb; iy++) {
      for (i = dc_get_index_for_coordinates(bord + absorb, iy, iz, sx, sy, sz);
           i <
           dc_get_index_for_coordinates(nx + bord + absorb, iy, iz, sx, sy, sz);
           i++) {
  ...
  for (iz = 0; iz < sz; iz++) {
    for (iy = 0; iy < sy; iy++) {
      for (ix = 0; ix < sx; ix++) {
        i = dc_get_index_for_coordinates(ix, iy, iz, sx, sy, sz);
        ...
        // random speed inside absortion zone
          ...
          vpz[i] = vpz[dc_get_index_for_coordinates(ivelx, ively, ivelz, sx, sy,
                       ...
                       (1.0 - bordDist) +
                   maxP * rfac * bordDist;
          vsv[i] = vsv[dc_get_index_for_coordinates(ivelx, ively, ivelz, sx, sy,
                                                    sz)] *
                       (1.0 - bordDist) +
                   maxS * rfac * bordDist;
        }
