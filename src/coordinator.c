...
problem_data_t dc_initialize_problem(MPI_Comm comm, unsigned int topology[DIMENSIONS], unsigned int workers, unsigned int iterations, unsigned int stencil_size, size_t size_x, size_t size_y, size_t size_z) {
  ...
  if(result.worker_indices == NULL) {
    dc_log_error(0, "Failed to allocate memory for worker indices.");
    free(result.cube);
    ...
    for(size_t x =  process_coordinates[0] * partition_size_x; x < process_coordinates[0] * partition_size_x + worker_size_x; x++) {
      for(size_t y = process_coordinates[1] * partition_size_y; y < process_coordinates[1] * partition_size_y + worker_size_y; y++) {
        for(size_t z = process_coordinates[2] * partition_size_z; z < process_coordinates[2] * partition_size_z + worker_size_z; z++) {
          size_t index = dc_get_index_for_coordinates(x, y, z, problem_data.size_x, problem_data.size_y, problem_data.size_z);
          problem_data.workers[worker][problem_data.worker_count[worker]] = problem_data.cube[index];
          problem_data.worker_indices[worker][problem_data.worker_count[worker]++] = dc_get_index_for_coordinates(x, y, z, problem_data.size_x, problem_data.size_y, problem_data.size_z);
        }
      }
    }
  }
