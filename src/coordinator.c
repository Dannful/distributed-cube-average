...
problem_data_t dc_initialize_problem(MPI_Comm comm,
  ...
  result.cube = (float *)malloc(arguments.size_x * arguments.size_y *
                                arguments.size_z * sizeof(float));
  const size_t size_x =
  ...
  result.pp = (float *)calloc(size_x * size_y * size_z, sizeof(float));
  result.pc = (float *)calloc(size_x * size_y * size_z, sizeof(float));
  result.qp = (float *)calloc(size_x * size_y * size_z, sizeof(float));
  result.qc = (float *)calloc(size_x * size_y * size_z, sizeof(float));
...
void dc_send_data_to_workers(problem_data_t problem_data) {
  for (size_t worker = 0; worker < problem_data.num_workers; worker++) {
    ...
    MPI_Send(problem_data.workers[worker], count, MPI_FLOAT, worker, 0,
             problem_data.communicator);
