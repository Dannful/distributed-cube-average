...
extern "C" void dc_propagate(const size_t start_coords[DIMENSIONS],
  ...
  cudaMemcpy(data->d_start_coords, start_coords, sizeof(size_t) * DIMENSIONS,
             cudaMemcpyHostToDevice);
  cudaMemcpy(data->d_end_coords, end_coords, sizeof(size_t) * DIMENSIONS,
             cudaMemcpyHostToDevice);
  cudaMemcpy(data->d_sizes, sizes, sizeof(size_t) * DIMENSIONS,
             cudaMemcpyHostToDevice);
  cudaMemcpy(data->d_process_coordinates, process_coordinates,
             sizeof(int) * DIMENSIONS, cudaMemcpyHostToDevice);
  cudaMemcpy(data->d_topology, topology, sizeof(int) * DIMENSIONS,
             cudaMemcpyHostToDevice);
  ...
  propagate_kernel<<<numBlocks, threadsPerBlock>>>(
      data->d_start_coords, data->d_end_coords, data->d_sizes, data->d_process_coordinates, data->d_topology,
