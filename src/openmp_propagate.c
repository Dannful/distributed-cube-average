...
void dc_propagate(
    ...
    const dc_precomp_vars *precomp_vars, const float dx, const float dy,
    const float dz, const float dt, float *pp_out, float *pc, float *qp_out,
    float *qc, const float *pp_in, const float *qp_in) {
  ...
  for (size_t z = start_coords[2]; z < end_coords[2]; z++) {
    for (size_t y = start_coords[1]; y < end_coords[1]; y++) {
      for (size_t x = 4; x < sizes[0] - 4; x++) {
        sample_compute(x, y, z, sizes, global_sizes, process_coordinates,
                       topology, dx, dy, dz, dt, pc, qc, pp_in, qp_in, pp_out,
                       qp_out, (dc_precomp_vars *)precomp_vars);
      }
    }
  }
}
